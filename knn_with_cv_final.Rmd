...
```{r}
# Cross validation
knn_cv_tune <- function(data, target, k_folds, k_range, distance_function) {
  set.seed(123)
  ...
  k_accuracies <- numeric(length(k_range))
  names(k_accuracies) <- as.character(k_range)
  ...
  for (k in k_range) {
    ...
    for (i in 1:k_folds) {
      ...
      # Run predictions
      predictions <- knn(train_data, train_labels, test_data, k, distance_function)
      accuracies[i] <- sum(predictions == test_labels) / length(test_labels)
    }
    # Store average accuracies
    k_accuracies[as.character(k)] <- mean(accuracies)
  }
...
k_range <- 1:9
k_accuracies <- knn_cv_tune(penguins, "species", k_folds = 5, k_range, euclidean_distance)

# Find the best k value
best_k <- which.max(k_accuracies)
